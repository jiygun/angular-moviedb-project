...
export class SeriesListComponent implements OnInit,DoCheck {
  ...
  pageSeriesId:number=Statics.ShowPathIds.seriesPathId;

  private _genresList:Array<Genres>;
  @Input() categoryFilters:Array<Genres>;
  
  constructor(private seriesService:SeriesService,private genresService:GenresService,private router:Router,private activatedRoute:ActivatedRoute,private componentFactoryResolver:ComponentFactoryResolver,private viewContainerRef:ViewContainerRef) {
  ...
  private setShowLit(){
    this.activatedRoute.queryParamMap.subscribe((result:any)=>{
      ...
      this.seriesService.getSeriesListWithFilters(this.pageNumber,this.getCategoryQuery(),this._filterQuery).subscribe((result:Result)=>{
        if(result.totalPages!=0){
          this._showList=result.showSummary;
          this._totalPages=result.totalPages;
          this.router.navigate(['/series'],{queryParams:{page:this.pageNumber>result.totalPages?result.totalPages:this.pageNumber}});
        }else{
          this._showList=new Array();
          this._totalPages=result.totalPages;
        }
  ...
  private setGenresList(){
    this.genresService.getSeriesGenres().subscribe((genres:Array<Genres>)=>{
  ...
  activePage($event){
    ...
    this.router.navigate([`/${Statics.UrlPaths.seriesPath}`],{queryParams:{page:this.pageNumber}});
  }
}
